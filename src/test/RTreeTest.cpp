...
namespace RTreeTest
{
    ...
    void printResultSummary(const std::vector<RTree::Data *> &results, int expectedCount)
    {
        ...
        if (!matches)
        {
            std::cerr << "  - Difference: " << (static_cast<int>(results.size()) - expectedCount) << " points" << std::endl;
    ...
    void runAllTests()
    {
        ...
        std::cout << "test with 1000000 points" << std::endl;
        // performanceTest(rtree, 1000000, 0.0, 100.0, 0.0, 100.0);
    }
